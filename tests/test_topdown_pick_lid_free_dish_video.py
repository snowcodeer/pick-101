"""Top-down petri lid pick video capture with free-floating dish base."""
import sys
...
def record_topdown_pick_lid_free_dish(lid_pos, renderer, cam, frame_interval=5, scene_option=None):
    """Record top-down lid pick sequence with free-floating dish base."""
    frames = []
...
def main():
    ...
    print(f"=== Top-Down Lid Pick Video (Free Dish, {args.angle} angle) ===")
    frames, success = record_topdown_pick_lid_free_dish(
        lid_pos,
        renderer,
        cam,
        frame_interval=5,
    ...
    if args.output:
        ...
        video_path = output_dir / f"topdown_pick_lid_free_dish_{args.angle}_{timestamp}.mp4"
