"""Evaluate a trained Cartesian petri lid lift policy with video recording."""
import argparse
...
def main():
    ...
    for ep in range(args.episodes):
        ...
        for step in range(max_episode_steps):
            ...
            i = info[0]
            act = action[0]
            log_line = (f"  step={step:3d}: z={i.get('lid_z', 0):.4f}, "
                        f"r={reward[0]:+.3f}, "
        ...
        ep_summary = (f"Episode {ep + 1}: reward={ep_reward:.2f}, success={is_success}, "
                      f"lid_z={final_info.get('lid_z', 0):.3f}")
        ep_summary += f", gripper_to_lid={final_info.get('gripper_to_lid', 0):.3f}"
        print(ep_summary)
